...
type ServiceFormState = Partial<Service>;
...
export default function AdminServices() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: ServiceFormState }) =>
      adminUpdateService(id, payload),
    onSuccess: () => {
  ...
  return (
    <AdminLayout title="Services">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">Kelola layanan yang ditampilkan di website.</p>
            ...
            <TableBody>
              {data?.map((service) => (
                <TableRow key={service.id}>
                  ...
                  <TableCell className="font-medium text-navy">{service.title}</TableCell>
                  <TableCell className="max-w-md text-muted-foreground">
                    <span className="line-clamp-1">{service.short_description}</span>
                  </TableCell>
                  <TableCell>
